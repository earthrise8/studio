...
function SubmitButton() {
  ...
  return (
    <Button type="button" className="w-full" disabled={pending} onClick={async () => {
        const provider = new GoogleAuthProvider();
        try {
            const result = await signInWithPopup(auth, provider);
            const idToken = await result.user.getIdToken();
            // This button click needs to submit the form
            document.getElementById('google-signin-form')?.requestSubmit();
        } catch (error) {
            console.error("Google sign in error", error);
            // The action state will handle displaying this error
             const form = document.getElementById('google-signin-form') as HTMLFormElement;
             const errorInput = document.getElementById('error-message-input') as HTMLInputElement;
             if(errorInput) {
                errorInput.value = 'Failed to sign in with Google. Please try again.';
             }
             form.requestSubmit();
        }
    }}>
